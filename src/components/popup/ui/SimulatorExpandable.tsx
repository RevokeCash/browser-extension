...
export default function SimulatorExpandable({ darkMode }: { darkMode: boolean }) {
  ...
  const [infoDescription, infoHeading, infoFirstTime, infoWarningsOnly, infoEveryTx] = useMemo(
    () => [
      t('popup.simulator.info.description'),
      t('popup.simulator.info.display_modes_title'),
      t('popup.simulator.info.first_time'),
      t('popup.simulator.info.warnings_only'),
      t('popup.simulator.info.every_transaction'),
    ],
    [t],
  );
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex flex-col flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              <div className="text-[13px] font-semibold text-neutral-100 break-words">{t('popup.simulator.title')}</div>
              <button
                type="button"
                onClick={() => setShowInfo(!showInfo)}
                className="flex-shrink-0 text-neutral-400 hover:text-neutral-200 transition-colors"
                aria-label={t('popup.simulator.info_button_aria')}
              >
            ...
            <div className="text-[12px] leading-snug break-words text-neutral-400">{t('popup.simulator.subtitle')}</div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {enabled && (
              <button
                ...
                aria-label={t('popup.simulator.expand_options_aria')}
              >
                <svg
                  width="20"
                  height="20"
            ...
            <button
              ...
              aria-label={t('popup.simulator.switch_aria')}
        ...
        {enabled && expanded && (
          <div className="mt-3 space-y-1">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-neutral-500 px-3 mb-2">
              {t('popup.simulator.display_mode_heading')}
            </div>
            <SimulatorModeOption
              id="once-per-dapp"
              title={t('popup.simulator.options.mixed.title')}
              description={t('popup.simulator.options.mixed.description')}
            ...
            <SimulatorModeOption
              id="warnings-only"
              title={t('popup.simulator.options.warnings_only.title')}
              description={t('popup.simulator.options.warnings_only.description')}
              isSelected={!showEveryTx && !!warningsOnly}
              onSelect={() => {
                setShowEveryTx(false);
            ...
            <SimulatorModeOption
              ...
              title={t('popup.simulator.options.every_transaction.title')}
              description={t('popup.simulator.options.every_transaction.description')}
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-[14px] font-semibold text-neutral-100">{t('popup.simulator.title')}</h3>
              <button
                onClick={() => setShowInfo(false)}
            ...
            <div className="text-[12px] leading-relaxed text-neutral-300 whitespace-pre-line space-y-2">
              <p>{infoDescription}</p>
              <div>
                <span className="block font-semibold text-neutral-100">{infoHeading}</span>
                <span className="block mt-2">{infoFirstTime}</span>
                <span className="block mt-2">{infoWarningsOnly}</span>
                <span className="block mt-2">{infoEveryTx}</span>
              </div>
            </div>
          </div>
        </div>
