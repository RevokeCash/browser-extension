...
// ROSCO: updated this so that it doesn't call the non-existent server side api
const getUnderlyingLogsProvider = (chainId: number): ViemLogsProvider => {
  // Extension build: always use client-side provider to avoid nonexistent backend API
