...
import { Address as ConstantsAddress, OpenSeaItemType } from '../constants';
import { NftListingItem, TokenData as BasicTokenData } from '../types';

import { ChainId } from '@revoke.cash/chains';
import { ERC20_ABI, ERC721_ABI } from '../abis';
import { DUMMY_ADDRESS, DUMMY_ADDRESS_2, WHOIS_BASE_URL } from '../constants';
import type { Contract, Nullable } from '../interfaces';
import ky from '../ky';
import {
  type Address,
  domainSeparator,
...
export const getNftListingItemTokenData = async (
  ...
  const tokenData = await getBasicTokenData(item.token, chainId);
  ...
  if (item.token === ConstantsAddress.ZERO) {
...
export const getBasicTokenData = async (address: Address, chainId: number): Promise<BasicTokenData> => {
